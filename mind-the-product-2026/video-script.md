...
6. Move through `Brief`, `Flows`, `Risks`, `Resilience`, `Evidence`, and `Pitch`.
7. Click `Copy packet`.
...
0:55-1:05 - Open `Resilience`. Show the pressure case, recovery move, evidence signal, and no-ship condition.

1:05-1:20 - Open `Evidence`. Show the hackathon scorecard, behavior coverage, and Novus/Pendo event map.

1:20-1:30 - Click `Copy packet`, then show the Novus/Pendo dashboard screenshot and close on measurable shippedness.
...
When I generate a launch packet, LaunchProof creates a concise brief, critical user flows, acceptance checks, launch risks, a resilience stress test, evidence signals, and a pitch-ready export. The Evidence tab also includes a hackathon scorecard mapped directly to the judging criteria: product thinking, craft, originality, and shippedness.

The Resilience tab names a pressure case, recovery move, proof signal, and no-ship condition. The Evidence tab maps user actions to Novus/Pendo events, including packet generation, flow review, risk review, resilience review, evidence review, and export. It also shows behavior coverage for the current session. That means LaunchProof is not only a demo; it is measurable as a shipped product.
...
- The generated `launchproof-demo-video.webm` is captioned; re-run the script after UI changes before final upload.
...
- Show the `Resilience` tab before `Evidence` so the World Product Day theme is part of the product, not only the writeup.
